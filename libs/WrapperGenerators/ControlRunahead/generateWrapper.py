...
def main(argv):
    ...
    noProtect = False
    try:
            opts, args = getopt.getopt(argv, "ho:i:w:n:d", ["orig=", "CFSlice=", "wrapper=", "np", "dmr"])
    except getopt.GetoptError:
            print 'Usage: python generateWrapper.py --origi original.v --CFSlice cfslice.v --wrapper output.v'
            sys.exit(2)

    for opt, arg in opts:
        if opt == '-h':
            print 'Usage: python generateWrapper.py --origi original.v --stitchup stitchup.v --wrapper output.v'
            sys.exit()
        if opt in ("-o", "--orig"):
            original = arg 
